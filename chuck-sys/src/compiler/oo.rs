...
use bitflags;
use crate::def::*;
use crate::compiler::compiler::typeck::*;

// Class hierachy
// Chuck_Object->Chuck_Event->Chuck_IO->Chuck_IO_Cherr/Chuck_IO_Chout/Chuck_IO_File
// Chuck_Object->Chuck_VM_Object
// Chuck_Object->Chuck_String

pub struct Chuck_VTable {
    pub funcs: Vec::<*mut Chuck_Func>,
}

pub struct Chuck_String {
    m_charptr: *const c_char,
    m_str: c_char,
}
pub struct Chuck_Event;
pub struct Chuck_IO {
    m_async_Event: *mut Chuck_Event,
    m_thread: *mut XThread,
}

pub struct Chuck_VM_Object {
    pub m_ref_count: t_CKUINT, //reference count
    pub m_pooled: t_CKBOOL, //if true, this allocates from a pool
    pub m_locked: t_CKBOOL, //if true, this should never be deleted
    pub m_v_ref: *mut Vec::<*mut Chuck_VM_Object>,
    pub our_locks_in_effect: t_CKBOOL,
}
impl Chuck_VM_Object {
    fn new(this: &Self) { this.init_ref(); }
    fn destroy(cvmo: Chuck_VM_Object) { drop(cvmo); }
    fn add_ref() { c_void; }
    fn release() { c_void; }
    fn lock() { c_void; }
    fn lock_all() { c_void; }
    fn unlock_all() { c_void; }
    fn init_ref() { c_void; }
}
...
impl Chuck_VM_Object for Chuck_Object {
    fn new() {
        Chuck_Object.new();
    }
    fn destroy() {
...
impl Chuck_Object for Chuck_String {
    fn new(s: &c_char) { set(s); }
    fn destroy(cs: Chuck_String) { drop(cs); }
    fn set(s: &c_char) {
        m_str = s;
        m_charptr = m_str.c_str();
    }
    fn str() { m_str; }
    fn c_str() { m_charptr; }
}
